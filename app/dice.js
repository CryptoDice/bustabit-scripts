...
var BetBoxMaxProfit = React.createClass({
  ...
  /*  var innerNode;
    ...
    switch(worldStore.state.coin_type){
      case 'BTC':
      case 'BITS':
        maxprofit = (worldStore.state.bankroll.btc.balance * 0.005);
        break;
      case 'LTC':
        maxprofit = (worldStore.state.bankroll.ltc.balance * 0.005);
        break;
      case 'DASH':
        maxprofit = (worldStore.state.bankroll.dash.balance * 0.005);
        break;
        case 'ADK':
        maxprofit = (worldStore.state.bankroll.adk.balance * 0.005);
        break;
        case 'GRLC':
        maxprofit = (worldStore.state.bankroll.grlc.balance * 0.01);
        break;
        case 'FLASH':
        maxprofit = (worldStore.state.bankroll.flash.balance * 0.01);
        break;
        case 'ETH':
        maxprofit = (worldStore.state.bankroll.eth.balance * 0.005);
        break;
        case 'MBI':
        maxprofit = (worldStore.state.bankroll.mbi.balance * 0.005);
        break;
        case 'WAVES':
        maxprofit = (worldStore.state.bankroll.waves.balance * 0.005);
        break;   
      case 'DOGE':
        maxprofit = (worldStore.state.doge_invest_app.maxprofit * 1).toFixed(2);
        break;
      case 'BXO':
        maxprofit = (worldStore.state.bxo_invest_app.maxprofit * 1).toFixed(4);
        break;
        case 'CLAM':
        maxprofit = (worldStore.state.clam_invest_app.maxprofit * 1).toFixed(8);
        break;          
    }
    ...
    return el.div(
      ...
      el.span({className: 'text'},
        ((worldStore.state.coin_type == 'DOGE')||(worldStore.state.coin_type == 'BXO')||(worldStore.state.coin_type == 'CLAM')) ?  maxprofit + ' ' + worldStore.state.coin_type : helpers.convSatstoCointype(maxprofit).toString() + worldStore.state.coin_type
      )
      //innerNode
    );
  }
...
var BetBoxWager = React.createClass({
  ...
  _validateWager: function(newStr) {
    ...
    } else if (num * 100 > worldStore.state.user.balances.btc) {
  ...
  }else if (worldStore.state.coin_type === 'DOGE'){
    if (isNaN(num) || !isFloatRegexp.test(newStr)) {
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'INVALID_WAGER' });
      // Ensure wager is greater than 0.01
      ...
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'WAGER_TOO_LOW' });
      // Ensure balance is enough
    } else if (num > worldStore.state.user.balances.doge) {
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'CANNOT_AFFORD_WAGER' });
    ...
    if (isNaN(num) || !isFloatRegexp.test(newStr)) {
      ...
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'WAGER_TOO_LOW' });
      // Ensure balance is enough
    } else if (num > worldStore.state.user.balances.bxo) {
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'CANNOT_AFFORD_WAGER' });
    } else if (helpers.getPrecision(num) > 8) {
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'WAGER_TOO_PRECISE' });
      // multiplier str is valid
    ...
    if (isNaN(num) || !isFloatRegexp.test(newStr)) {
      ...
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'WAGER_TOO_LOW' });
      // Ensure balance is enough
    } else if (num > worldStore.state.user.balances.clam) {
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'CANNOT_AFFORD_WAGER' });
    } else if (helpers.getPrecision(num) > 8) {
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'WAGER_TOO_PRECISE' });
      // multiplier str is valid
    ...
    if (isNaN(num)) {
      ...
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'WAGER_TOO_LOW' });
      // Ensure balance is enough
    } else if (num / 0.00000001 > worldStore.state.user.balances[worldStore.state.coin_type.toLowerCase()]) {
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'CANNOT_AFFORD_WAGER' });
    } else if (helpers.getPrecision(num) > 8) {
      Dispatcher.sendAction('UPDATE_WAGER', { error: 'WAGER_TOO_PRECISE' });
      // multiplier str is valid
  ...
  _onMaxWager: function() {
    ...
    if (confirm("Confirm wager full balance?") == true) {
            var wager = 0;
            if (worldStore.state.user) {
              switch(worldStore.state.coin_type){
                case 'BITS':
                case 'BTC':
                  wager = worldStore.state.user.balances.btc - 1;
                  break;
                case 'LTC':
                  wager = worldStore.state.user.balances.ltc - 1;
                  break;
                case 'DASH':
                  wager = worldStore.state.user.balances.dash - 1;
                  break;
                  case 'ADK':
                  wager = worldStore.state.user.balances.adk - 1;
                  break;
                  case 'GRLC':
                  wager = worldStore.state.user.balances.grlc - 1;
                  break;
                  case 'FLASH':
                  wager = worldStore.state.user.balances.flash - 1;
                  break;
                  case 'ETH':
                  wager = worldStore.state.user.balances.eth - 1;
                  break;
                  case 'MBI':
                  wager = worldStore.state.user.balances.mbi - 1;
                  break;
                  case 'WAVES':
                  wager = worldStore.state.user.balances.waves - 1;
                  break;   
               case 'DOGE':
                  wager = worldStore.state.user.balances.doge - 0.01;
                  break; 
                case 'BXO':
                  wager = worldStore.state.user.balances.bxo - 0.001;
                  break;
                  case 'CLAM':
                  wager = worldStore.state.user.balances.clam - 0.00000001;
                  break;        
              }
              if (wager < 0){
                wager = 0;
            ...
            Dispatcher.sendAction('UPDATE_WAGER', { str: helpers.convNumtoStr(wager) });
        } else {
  ...
  render: function() {
    ...
    return el.div(
      ...
      el.div({className: 'input-group'},//style: { marginTop: '-15px' }},
        ...
        el.span(
          {className: 'input-group-addon', style:{fontWeight:'bold', color:'white'}},
          worldStore.state.coin_type
        )
      ),
      el.div(
...
var BetBoxLastBet = React.createClass({
 ...
 if (worldStore.state.first_bet){
   ...
   last_currency = worldStore.state.bets.data[worldStore.state.bets.end].currency;
      ...
      null,
        el.div(
          ...
          el.div(
            ...
            el.div(
                null,
               ((last_currency == 'DOGE')||(last_currency == 'BXO')||(last_currency == 'CLAM')) ? el.a(
                  {
                    href: window.location.href + 'bets/' + last_bet,
                    target: '_blank'
                  },
                  last_bet
                ) : el.a(
    ...
    if (worldStore.state.user) {
      if (stillAnimatingPucks||worldStore.state.rt_spin_running||worldStore.state.plinko_running)
        ...
        switch (worldStore.state.coin_type){
          case 'BTC':
          case 'BITS':
          b_balance = worldStore.state.user.balances.btc;
            break;
          case 'LTC':
          b_balance = worldStore.state.user.balances.ltc;
            break;
          case 'DASH':
            b_balance = worldStore.state.user.balances.dash;
            break;
            case 'ADK':
            b_balance = worldStore.state.user.balances.adk;
            break;
            case 'GRLC':
            b_balance = worldStore.state.user.balances.grlc;
            break;
            case 'FLASH':
            b_balance = worldStore.state.user.balances.flash;
            break;
            case 'ETH':
            b_balance = worldStore.state.user.balances.eth;
            break;
            case 'MBI':
            b_balance = worldStore.state.user.balances.mbi;
            break;
            case 'WAVES':
            b_balance = worldStore.state.user.balances.waves;
            break;
            case 'BXO':
            b_balance = worldStore.state.user.balances.bxo;
            break;  
          case 'DOGE':
            b_balance = worldStore.state.user.balances.doge;
          break; 
          case 'CLAM':
            b_balance = worldStore.state.user.balances.clam;
            break;
        }
...
var BetBoxButton = React.createClass({
  ...
  _makeBetHandler: function(cond) {
    ...
    return function(e) {
      if ((worldStore.state.confirm_btc != true)&&((worldStore.state.coin_type == 'BTC')||(worldStore.state.coin_type == 'BITS'))){
        var r = confirm("You are about to bet with BTC, are you sure?");
        if (r == true){
          Dispatcher.sendAction('CONFIRM_BTC',null);
        }
      }else{
        var r = true;
      }
    ...
    //  if ((worldStore.state.coin_type == 'DOGE')||(worldStore.state.coin_type == 'BXO')){
    ...
    socket.emit('dice_bet', params, function(err, bet) {
          console.log('Socket returned for dice', bet);
          self.setState({ waitingForServer: false });
          if (err){
            console.log('Error DICE BET', err);
            self.setState({ waitingForServer: false });
            ...
            if(err == 'INVALID_HASH '){
              ...
              alert('Dice Bet Error: ' + JSON.stringify(err) + ' Fetching new hash')  
            }
            gethashfromsocket();
            return;
          }else{
            var fair;
            if ((bet.currency =='DOGE')||(bet.currency =='BXO')||(bet.currency =='CLAM')){
              fair = CryptoJS.SHA256(bet.secret + '|' + bet.salt).toString() === hash
            }else{
              fair = CryptoJS.SHA256(bet.secret + bet.salt).toString() === hash
            }
              bet.meta = {
              ...
              hash: hash,
              kind: 'DICE',
              isFair: fair//CryptoJS.SHA256(bet.secret + bet.salt).toString() === hash
            };
            ...
            switch (worldStore.state.coin_type){
              ...
              case 'BITS':
                Dispatcher.sendAction('UPDATE_USER', {
                  balances: {btc: worldStore.state.user.balances.btc + bet.profit}
                });
              break;
              case 'LTC':
                Dispatcher.sendAction('UPDATE_USER', {
                  balances: {ltc: worldStore.state.user.balances.ltc + bet.profit}
                });
              break;
              case 'DASH':
                Dispatcher.sendAction('UPDATE_USER', {
                  balances: {dash: worldStore.state.user.balances.dash + bet.profit}
                });
              break;
              case 'ADK':
              Dispatcher.sendAction('UPDATE_USER', {
                balances: {adk: worldStore.state.user.balances.adk + bet.profit}
              });
              break;
              case 'GRLC':
                Dispatcher.sendAction('UPDATE_USER', {
                  balances: {grlc: worldStore.state.user.balances.grlc + bet.profit}
                });
              break;
              case 'FLASH':
              Dispatcher.sendAction('UPDATE_USER', {
                balances: {flash: worldStore.state.user.balances.flash + bet.profit}
              });
            break;
              case 'ETH':
              Dispatcher.sendAction('UPDATE_USER', {
                balances: {eth: worldStore.state.user.balances.eth + bet.profit}
              });
            break;
            case 'MBI':
            Dispatcher.sendAction('UPDATE_USER', {
              balances: {mbi: worldStore.state.user.balances.mbi + bet.profit}
            });
            break;
            case 'WAVES':
              Dispatcher.sendAction('UPDATE_USER', {
                balances: {waves: worldStore.state.user.balances.waves + bet.profit}
              });
              break;
              case 'BXO':
                Dispatcher.sendAction('UPDATE_USER', {
                  balances: {bxo: worldStore.state.user.balances.bxo + bet.profit}
                });
              break;
              case 'DOGE':
                Dispatcher.sendAction('UPDATE_USER', {
                  balances: {doge: worldStore.state.user.balances.doge + bet.profit}
                });
                break;
                case 'CLAM':
                Dispatcher.sendAction('UPDATE_USER', {
                  balances: {clam: worldStore.state.user.balances.clam + bet.profit}
                });
                break;
            }
      ...
      apiCall.placeDiceBet(params, {
        ...
        error: function(xhr) {
          console.log('Error DICE BET');

          self.setState({ waitingForServer: false });
          console.log('auto bet stopped from error');
          Dispatcher.sendAction('STOP_RUN_AUTO');
          if(xhr.responseJSON.error == 'INVALID_HASH '){
            alert('Bet Stopped due to INVALID_HASH, Fetching new hash automatically');
            gethashfromsocket();
          }
          return;
...
var AutobetButtons= React.createClass({
  ...
  _onClickStopWin: function(){
    Dispatcher.sendAction('TOGGLE_STOP_ON_WIN');
    this.forceUpdate();
  },
  render: function() {
    var winProb = helpers.multiplierToWinProb(betStore.state.multiplier.num);
...
render: function(){
  ...
  // TODO: Create error prop for each input
  var error = betStore.state.wager.error || betStore.state.multiplier.error || betStore.state.clientSeed.error || AutobetStore.state.lossmul.error || AutobetStore.state.winmul.error ||AutobetStore.state.stoplower.error || AutobetStore.state.stophigher.error;
  ...
  if (AutobetStore.state.Stop_on_win){
    var autostop = el.button({type: 'button',
      className: 'btn btn-md btn-default btn-block',
      onClick: this._onClickStopWin},
      el.span(
...
var DiceGameTabContent = React.createClass({
  ...
  render: function() {
    ...
    switch(worldStore.state.coin_type){
      case 'BITS':
        maxprofit = helpers.convSatstoCointype(worldStore.state.bankroll.btc.balance * 0.005);
        break;
      case 'BTC':
      case 'LTC':
      case 'DASH':
      ...
      case 'MBI':
      case 'WAVES':
        maxprofit = helpers.convSatstoCointype(worldStore.state.bankroll[worldStore.state.coin_type.toLowerCase()].balance * 0.005);
        break;
      case 'GRLC':
      case 'FLASH':
        maxprofit = helpers.convSatstoCointype(worldStore.state.bankroll[worldStore.state.coin_type.toLowerCase()].balance * 0.01);
        break; 
      case 'DOGE':
        maxprofit = (worldStore.state.doge_invest_app.maxprofit * 1).toFixed(2);
        break;
      ...
      el.div({className:'panel-heading'},
        ...
        el.div({className:'navbar-right', style: {marginRight:'5px'}},
          el.ul({className:'list-inline'},
            el.li({className:'hidden-xs'},' Max Profit: ' + maxprofit + ' ' + worldStore.state.coin_type),
            el.li({
                  id:'popover-btn',
                  //'data-html':'true',
                  'data-container':'body',
